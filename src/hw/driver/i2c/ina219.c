...
float    _powerMultiplier_mW =0.0f;
...
*/
/**************************************************************************/
bool wireReadRegister(uint8_t dev_addr, uint16_t *value)
{
	uint8_t buffer[2];
	bool ret;
	ret = i2cWriteData(_DEF_I2C2, dev_addr, value, 2, 100);
...
bool getCurrent_raw_int(int16_t * rawValue) {
  uint16_t value = 0;
  bool success = true ;
